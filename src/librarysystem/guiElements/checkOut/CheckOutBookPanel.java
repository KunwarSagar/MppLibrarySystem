...
import librarysystem.ConstantConfig;
import librarysystem.MessageBox;
import librarysystem.UIController;
import librarysystem.Util;
import librarysystem.guiElements.UtilGui;
import librarysystem.validation.*;
    ...
    private class checkOutListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) throws NumberFormatException {
            ...
            try {
                ...
                Validation checkoutRules = ValidationFactory.getValidation(CheckOutBookPanel.this);
                checkoutRules.checkValidation(CheckOutBookPanel.this);
                ...
                if( !ci.checkMemberId(memeberId))
                    throw new ValidationException("No member found with Member ID = "+ memeberId);

                // Check if book is available
                Book book = ci.checkBookISBN(isbn);
                if(book == null)
                    throw new ValidationException("No book with ISBN = "+ isbn + " found");
                ...
                DefaultTableModel model = (DefaultTableModel) UIController.INSTANCE.both.checkOutList.getModel();
                model.insertRow(0, new  Object[]{ member.getMemberId() , book.getIsbn(),  entry.getCheckOutDate(), entry.getDueDate()});

                // successful
                new MessageBox.InnerFrame().showMessageBox("Check out successfully completed", "Info");

            } catch (NumberFormatException ex){
                new MessageBox.InnerFrame().showMessageBox("Input for Max days should be a number", "Error");
            } catch (ValidationException | CheckOutException | LibraryMemberException | BookCopyException ex) {
